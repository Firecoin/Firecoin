...
uint256 hashGenesisBlock("0xe468abb1a70196fee93298a37ccc4b19579e7fab6a89dbe07738624716646209");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Firecoin: starting difficulty is 1 / 2^12
...
{
    // Firecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    else if (nHeight < 17281)
    { nSubsidy = 70 * COIN;}
    else if (nHeight < 60481)
    { nSubsidy = 30 * COIN;}
    else if (nHeight < 103681)
    { nSubsidy = 25 * COIN;}
    else if (nHeight < 146881)
    {nSubsidy = 21 * COIN;}
    else if (nHeight < 190081)
    {nSubsidy = 17 * COIN;}
    else if (nHeight < 233281)
    {nSubsidy = 14 * COIN;}
    else if (nHeight < 276481)
    {nSubsidy = 11 * COIN;}
    else if (nHeight < 319681)
    {nSubsidy = 9 * COIN;}
    else if (nHeight < 362881)
    {nSubsidy = 7 * COIN;}
    else if (nHeight < 406081)
    {nSubsidy = 6 * COIN;}
    else if (nHeight < 4554575)
    {nSubsidy = 5 * COIN;}
    else
    {nSubsidy =0;}
    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
   // nSubsidy >>= (nHeight / 840000); // Firecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 20 * 60; // Firecoin: 3.5 days
static const int64 nTargetSpacing = 1 * 60; // Firecoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Firecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Firecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Firecoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe468abb1a70196fee93298a37ccc4b19579e7fab6a89dbe07738624716646209");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Today,The Firecoin is building to world for now...";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1388999956;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085577905;
        ...
        assert(block.hashMerkleRoot == uint256("0xb66807b417c0e3d1c883183bd33244d44ba3e14ede3aea021e6513b0657922c9"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Firecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FirecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FirecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static FirecoinMiner(CWallet *pwallet)
{
    printf("FirecoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running FirecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FirecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FirecoinMiner, pwallet));
}
